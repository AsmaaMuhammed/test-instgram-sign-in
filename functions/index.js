...
admin.initializeApp(
  ...
  databaseURL: 'https://pidgeco-pidge2.firebaseio.com',
});

const OAUTH_REDIRECT_URI = 'https://test-instgram-signin.firebaseapp.com/popup.html';
  ...
  try {
    return cookieParser()(req, res, async () => {
      ...
      console.log('Auth code exchange result received:', results);
        ...
        const profilePic = "https://upload.wikimedia.org/wikipedia/commons/8/89/Portrait_Placeholder.png";//results.user.profile_picture;
        const userName = "unknown";//results.user.full_name;
...
async function createFirebaseAccount(instagramID, displayName, photoURL, accessToken) {
  ...
  const uid = `instagram:${instagramID}`;//console.log(uid);

  // Save the access token to the Firebase Realtime Database.
  const databaseTask = admin.database().ref(`users/instagramAccessToken/${uid}`).set(accessToken);
